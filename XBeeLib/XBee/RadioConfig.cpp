...
bool XBee::is_node_discovery_in_progress()
{
    ...
    //const int nd_timer = chrono::duration_cast<std::chrono::milliseconds>(_nd_timer.elapsed_time()).count();
...
void XBee::_get_remote_node_by_id(const char * const node_id, uint64_t * const addr64, uint16_t * const addr16)
{
    ...
    *addr16 = ADDR16_UNKNOWN;
    ...
    if (wait_for_complete_timeout) {
       ...
       //    while (chrono::duration_cast<std::chrono::milliseconds>(nd_timer.elapsed_time()).count()) {           
            //wait_ms(10);
            thread_sleep_for(10);
        }
    }
